...
5. Multiplayer: every SEAT is a person — an INSTANCE in one editor,
   played by its resident tester (`game.instance(id)` → `bot.goal`) or a
...
- **Author in playable slices, playtested LIVE by two people: you are
  ...
  tester's honest input at sim speed, directed by goal (`bot.goal`),
...
- **Keep Three source modular:** thin `src/world.tsx`; compositions in
  ...
  prefab-per-primitive. Characters are ALWAYS the rigged generator
