...
//         source, scripts, or data. A game that genuinely has no such concern
...
//     W1. setInterval/setTimeout in a file that also imports
//         useWorldState/WorldProvider, OR in a game-state UI file that owns
//         React state (`src/ui/**` and every manifest-declared dom root
//         entry) — looks like a manual polling pump. The second leg exists
//         because keying only on the import made the rule blind to the
//         pump that had stopped importing the hook.
//         to the live playtest loop and the editor panels.
...
//     W5. a humanoid rig / Three AnimationMixer / XState animation binding
//         clock tick inside a Behavior whose phase is not `animation` —
//         native animation must advance after corrected gameplay/physics state,
//         never from prePhysics merely because the controller owns the model.
//     W6. a Behavior init() detaches an async load/initialize/prepare
//         method with `void this.load...()` — required setup can then fail
//         after Play has already acknowledged readiness.
  ...
  for (const file of srcFiles) {
    ...
    for (const m of blanked.matchAll(re)) {
      ...
      addFinding(
        ...
        'error',
        'bot-canonical',
        "?bot= query-param plumbing found in src/** — the resident tester (bot.*) owns bot entry; hand-rolled bot flags are banned.",
        'Delete this bot-param handling; direct the resident tester from the live session (bot.goal).',
      );
    }
  }
}
...
// ---------------------------------------------------------------------------
// W1 — polling pump next to useWorldState/WorldProvider
// ---------------------------------------------------------------------------
...
/**
 * W1's scope, in two legs — and the second leg exists because the first one
 * could not see its own quarry.
 *
 * Measured miss: a probe shipped `window.setInterval(tick, 50)` driving React
 * state in a dom-root connector, and this rule said nothing, because it keyed
 * on PROXIMITY to a `useWorldState` import — and the offending file had stopped
 * importing it. Keying the pump on the presence of the thing it should have
 * used instead makes the rule blindest exactly where the drift is worst.
 *
 * So: the import leg stays (a file that imports the hook and polls anyway is
 * still the pump), and a UI leg is added that does not ask what the file
 * imports at all — `src/ui/**` by convention, plus every manifest-declared dom
 * root entry.
 */
function pollingPumpScope(file: string, blanked: string, domRootEntries: Set<string>): boolean {
  if (/\bimport\b[^;]*?\b(useWorldState|WorldProvider)\b[^;]*?\bfrom\b/.test(blanked)) return true;
...
function checkTriggerNameIdentity(): void {
  // Entity identity belongs to COMPONENTS (attach a marker Behavior and
  // check for it), not to the display name: `other.name === 'Coin'` works
      ...
      for (; j < lines.length; j++) {
        ...
        if (nameEqRe.test(line)) {
          addFinding(
            ...
            'Attach a marker Behavior to the entities you care about and gate on the component, not object.name.',
          );
        }
...
// ---------------------------------------------------------------------------
// W5 — native animation clocks tick only in the animation phase
// ---------------------------------------------------------------------------

function checkAnimationPhaseOwnership(): void {
  // Keep this deliberately structural and neutral: it does not prescribe a
  // controller, camera, state names, clip set, or character design. It only
  // catches clocks whose ordering is objectively wrong. State-machine policy
  // remains review-only because distinguishing gameplay locomotion from a
  // preview/cinematic by regex would create noisy, creativity-constraining
  // false positives.
  const classStartRe = /\bclass\s+\w+\s+extends\s+Behavior(?:<[^>]+>)?/;
  const animationPhaseRe = /\bstatic\s+phase\s*=\s*(?:['"]animation['"]|SystemPhase\.ANIMATION)/;
  const animationTickRe =
    /\b(?:this\.)?(?:rig|mixer|animationMixer|animBinding|animationBinding)\s*\??\.\s*(?:update|tick)\s*\(/;
  ...
  for (const file of srcFiles) {
    ...
    while (i < lines.length) {
      ...
      if (!animationPhaseRe.test(classText)) {
        classLines.forEach((line, offset) => {
          if (!animationTickRe.test(line)) return;
          addFinding(
            file,
            rawLines,
            i + offset + 1,
            'W5',
            'warn',
            'animation-phase-ownership',
            'an animation clock is advanced by a Behavior outside the animation phase, so it can sample pre-correction movement/physics state.',
            "Move animation ownership to a separate `static phase = 'animation'` Behavior; let it read the controller's corrected state, drive XState, and tick bindXStateAnimation there.",
          );
        });
      }
      i = Math.max(j + 1, i + 1);
...
// ---------------------------------------------------------------------------
// W6 — required async initialization must be awaited by Behavior.init
// ---------------------------------------------------------------------------

function closingBraceAfter(text: string, start: number): number {
  let depth = 0;
  let opened = false;
...
function checkDetachedComponentInitialization(): void {
  const initStartRe = /\binit\s*\([^)]*\)\s*(?::\s*[^{]+)?\s*\{/g;
  const detachedLoadRe = /\bvoid\s+this\.(?:load|initialize|prepare|setup)[A-Z][A-Za-z0-9_]*\s*\(/g;
  for (const file of srcFiles) {
    const { rawLines, blanked } = readFileText(file);
    for (const initMatch of blanked.matchAll(initStartRe)) {
      const end = closingBraceAfter(blanked, initMatch.index);
      const body = blanked.slice(initMatch.index, end);
      for (const detached of body.matchAll(detachedLoadRe)) {
        const absoluteIndex = initMatch.index + detached.index;
        addFinding(
          file,
          rawLines,
          lineNumberAt(blanked, absoluteIndex),
          'W6',
          'warn',
          'await-component-initialization',
          'Behavior.init() detaches a load-like promise, so Play can acknowledge readiness before required setup succeeds or fails.',
          'Make init async and await/return the promise (for example `async init() { await this.loadClips(); }`). Use an idioms-ignore reason only when the work is genuinely optional.',
        );
      }
    }
  }
...
// ---------------------------------------------------------------------------
// E6 — registered project-tool starter sentinel left in place
// ---------------------------------------------------------------------------
  ...
  for (const file of toolFiles) {
    ...
    for (const match of blanked.matchAll(sentinel)) {
      addFinding(
        ...
        'TOOL_NOT_CONFIGURED remains in a project tool — this is an explicit starter sentinel, not a usable implementation.',
        'Replace the starter schema and implementation with game-specific behavior, remove the sentinel error declaration, and exercise the tool before claiming completion.',
      );
    }
  }
}
...
// ---------------------------------------------------------------------------
// E7 — standard project-tool handles remain explicitly registered
// ---------------------------------------------------------------------------

function projectToolEntry(registration: unknown): string | null {
  if (typeof registration === 'string') return registration;
  if (!registration || typeof registration !== 'object') return null;
  const entry = (registration as { entry?: unknown }).entry;
  return typeof entry === 'string' ? entry : null;
}

function registeredProjectToolNames(registrations: unknown[]): Set<string> {
  const names = new Set<string>();
  for (const registration of registrations) {
    const entry = projectToolEntry(registration);
    if (!entry) continue;
    const entryPath = resolve(rootDir, entry);
    if (!existsSync(entryPath)) continue;
    const { blanked } = readFileText(entryPath);
    for (const match of blanked.matchAll(/\bname\s*:\s*['"]([^'"]+)['"]/g)) {
      names.add(match[1]!);
    }
  }
  return names;
...
function checkPrefabStories(): void {
  const prefabs = srcFiles.filter((file) => {
    const rel = relPath(file);
    return (
  ...
  for (const file of prefabs) {
    ...
    const story = STORY_EXTENSIONS.map((extension) =>
      join(dirname(file), `${name}${extension}`),
...
checkAnimationPhaseOwnership();
checkDetachedComponentInitialization();
