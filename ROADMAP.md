...
Completion:
- The project-local sections live in the workbench's workspace storage scope, and that scope lives in the project's folder: the fork's `workspaceStorageUrl` (volter-ai/code-oss `50214fb5`) keeps it in `.vgai/workbench-storage.json`, and the frame hands the scope to the kit at mount. Remaining: a workbench release cut from that pin, walked (the layout and pins survive a folder rename and another browser).
