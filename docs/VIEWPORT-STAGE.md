...
|---|---|---|
| Blender | Solid (its own studio, AgX, no environment), the fill, outline, grid step and widths; Material Preview (its world studio lights as environment images, Forest at strength 1 fixed in the world over the fill, the scene's lamps left out); Rendered (the scene's own lights, its World and shadows, the lighting the document's render photographs with, held on the viewport while its view lights by `scene`); Blender's four shading cells as one group, each shading type keeping its own lighting (the stage states them per draw mode: `solid`, `preview`, `rendered`); the 3D cursor (`Scene.cursor`, drawn as Blender's overlay draws it and placed with Shift+Right-click) and the selected objects' origin dots | box and wire on Blender documents (its selection ids are datablocks, not three objects) |
...
- Blender passes on a Front Orthographic frame (`captureEditorChrome({ region: 'document' })`) against `gizmo-select-box.png`. Its first round failed on a missing grid and X axis in that view and an oversized origin dot; the grid now lies in an axis-aligned orthographic view's own plane and the dot is Blender's size. Remaining, most important first:
  ...
  - the subject line reads `cube | Cube` where Blender reads `(1) Collection | Cube`, and there is no grid-scale line (`10 Centimeters`). (On a cold open the active object used to be missing from it, and the Move gizmo and origin dot with it: the stage's content swap dropped a selection the engine had published in the presentation's ids. Fixed; it keeps any id its adapter resolves.)
  - the grid's lines are brighter and closer in weight than Blender's, and the X axis thicker and more saturated;
  - the navigation gizmo's colours are fixed values fitted to the default view, where Blender's are computed (`view3d_gizmo_navigate_type.cc`): a ball's fill is the theme axis colour mixed with the viewport background by `(depth+1)·0.25+0.5` (the front view's X fill measures (204,55,78), exactly that mix at depth 0); a negative ball is a 25% tint with a faded ring; in an axis-aligned view the far ball of that axis is not drawn and the near negative one is filled, ringed halfway to white and lettered (`-Y`); letters are black at 0.9. The ball form is shared with Godot's look, which passes, so this is its own unit;
  - the Select Box tool shows a plain arrow;
  - the outline's corners are slightly rounded and its orange duller;
  - Solid is now drawn by Blender's own shading function (`@volter/blender-engine` `blender-workbench-material.ts`, transcribed from `workbench_world_light_lib.glsl` over the factory studio lights and each material's viewport display). Evaluated offline under AgX it predicts 161 for that face and the default view's faces at 149 / 141 / 125; drawn, they read 158–160 and 148 / 139 / 125. The older default-view reference frames (141 / 129 / 111) match the same function under the Standard transform instead (147 / 136 / 116), so they were made under Standard; Blender's factory transform is AgX. Where evidence stops: the front face is the one AgX-made reference, and no default-view frame under AgX has been taken yet.
