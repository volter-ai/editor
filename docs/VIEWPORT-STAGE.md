...
|---|---|---|
| Blender | Solid (its own studio, AgX, no environment), the fill, outline, grid step and widths; Material Preview (its world studio lights as environment images, Forest at strength 1 fixed in the world over the fill, the scene's lamps left out); Rendered (the named view `blender-rendered`: the scene's own lights, its World and shadows, the lighting the document's render photographs with, held on the viewport while its view lights by `scene`); the 3D cursor (`Scene.cursor`, drawn as Blender's overlay draws it and placed with Shift+Right-click) and the selected objects' origin dots | Blender's four shading cells as one exclusive group: the segments set the draw mode alone, so Solid after Material Preview or Rendered keeps that view's lighting until the Lighting row changes it; box and wire on Blender documents (its selection ids are datablocks, not three objects) |
