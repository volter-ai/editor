...
|---|---|---|
| Blender | Solid (its own studio, AgX, no environment), the fill, outline, grid step and widths; Material Preview (its world studio lights as environment images, Forest at strength 1 over the fill) | Material Preview's light turning with the view (World Space Lighting off; here it stays in the world), Rendered (the engine's render lighting is not a scene light the stage can switch to), box and wire on Blender documents (its selection ids are datablocks, not three objects) |
...
Neither half is accepted: Unreal and Blender above, Unreal's capability row, and Blender's view-turned Material Preview light and Rendered mode. The reference frames are in `/Volumes/PeakSSD/volter-work/engine-reference`; the world stage is compiled but not yet seen on a project with a world.
