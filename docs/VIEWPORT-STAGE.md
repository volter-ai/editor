...
|---|---|---|
| Blender | Solid (its own studio, Standard, no environment), the fill, outline (Blender's outline detect: a hard 4 device px band on both sides of the silhouette), grid (one-pixel lines and Blender's three levels in an axis-aligned orthographic view), axis lines; Material Preview (its world studio lights as environment images, Forest at strength 1 fixed in the world over the fill, the scene's lamps left out); Rendered (the scene's own lights, its World and shadows, the lighting the document's render photographs with, held on the viewport while its view lights by `scene`); Blender's four shading cells as one group, each shading type keeping its own lighting (the stage states them per draw mode: `solid`, `preview`, `rendered`); the 3D cursor (`Scene.cursor`, placed with Shift+Right-click) and the selected objects' origin dots; the overlay text (view name, `(frame) Collection \| Object` from the engine, and the grid step's name in the scene's units); the lens (36 mm over 50 mm at the viewport's zoom of 2) and the file's saved 3D View as the opening view; the navigation cluster (Zoom and Pan as drags, Camera, and the projection mark); the camera view (numpad 0 and the cluster's camera: the scene camera's frame, passepartout and borders, frame zoom and pan, leaving on rotate); the overlay's extras (cameras, lights, empties) | box and wire on Blender documents (its selection ids are datablocks, not three objects); the camera view's lock button; a spot light's cone, the arrows' end markers and image empties; picking an extra by clicking its wire; a saved view's roll and lens |
...
- Blender passes on a Front Orthographic frame (`captureEditorChrome({ region: 'document' })`) against `gizmo-select-box.png`. Its judge's objections are closed, each by Blender's own rule and measured against that frame at the same scale (198 px per metre):
  - the navigation cluster is Blender's four buttons (`view3d_gizmo_navigate.cc`): Zoom and Pan as drags (Zoom by the factory Dolly rule, `viewzoom_scale_value`), Camera, and the projection toggle whose mark is the projection the view has, never pressed. Framing is not in it: its home is the document header's view control and Home / numpad period, as Blender's is its View menu;
  - the overlay reads `Front Orthographic`, `(1) Collection | Cube` and `10 Centimeters`, as Blender's does: the subject line is composed by the engine (`draw_selected_name`), its frame and marker following the playhead, and the grid step is named in the scene's units (`ED_view3d_grid_view_scale`);
  - an axis-aligned orthographic grid draws Blender's three levels in one-pixel lines: 10 cm lines +6 over the ground (Blender +5), 1 m lines +26 (+26), the X axis one pixel (203, 41, 63) against (198, 43, 63);
  - the navigation gizmo's colours are computed as Blender computes them; +X (246, 51, 81) against Blender's (245, 54, 81), with the aligned view's `-Y` ball;
  - the Select Box tool is Blender's dashed orange marquee over the pointer;
  - the outline is Blender's outline detect, a hard band 4 device pixels wide across the silhouette's edge, (255, 160, 40) against (255, 158, 44).
- Blender's Solid is drawn by Blender's own shading function (`@volter/blender-engine` `blender-workbench-material.ts`, transcribed from `workbench_world_light_lib.glsl` over the factory studio lights and each material's viewport display), under the Standard view: Blender gives Solid the display's default view and none of the scene's settings (`draw_color_management.cc`), and the sRGB display's default is Standard (`config.ocio`); the scene's AgX is Material Preview's and Rendered's. Blender 5.2's Workbench rendered under Standard gives the factory cube 142 / 131 / 112 in the default view and 162 on the front face, agreeing with the viewport's own frames (141 / 129 / 111, 161); drawn here they read 145 / 133 / 115 and 159–161.
- Blender's opening view is the file's saved 3D View (the Modeling workspace's `RegionView3D`), and its lens is the view's arithmetic, 36 mm over 50 mm at `CAMERA_PARAM_ZOOM_INIT_PERSP` 2 (71.5°); the factory cube's share of the region then matches Blender's frame within 2%.
- Blender's camera view (`view3d.view_camera`) is the navigation cluster's camera and numpad 0: the scene camera's view with its frame at Blender's `camzoom` 0, the passepartout and the frame's borders, the wheel, a pan and a dolly zooming and moving the frame by Blender's rules, and rotating leaving at the camera's pose. Numpad 0 is in the generated workbench keymap and reaches a workbench built from it.
- Blender's overlay extras draw a scene's cameras, lights and empties (`blender-runtime-extras.ts`); against `modeling-object-none.png` the factory camera and point light draw with Blender's shapes and places.
