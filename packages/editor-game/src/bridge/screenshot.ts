...
function screenshotRecordingNotice(): GameCaptureRecordingNote | null {
  ...
  return {
    ...
    notice: [
      ...
      '',
      `  recording: ${live.path}${live.rotates ? '   (the next `vgai play` replaces this file)' : '   (named — never rotated)'}`,
      ...
      '',
      'The clip is finalized and readable when play stops — `vgai stop`, or the idle',
