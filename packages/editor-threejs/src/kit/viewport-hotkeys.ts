...
export function registerViewportHotkeys(
  ...
  const unbind = keyboard.bindActions([
    ...
    { id: 'view.bottom', scope: 'stage', run: () => store.shell.setViewPreset('bottom') },
    { id: 'view.back', scope: 'stage', run: () => store.shell.setViewPreset('back') },
    { id: 'view.left', scope: 'stage', run: () => store.shell.setViewPreset('left') },
    { id: 'view.orbitLeft', scope: 'stage', run: () => store.shell.stepView('orbit-left') },
    { id: 'view.orbitRight', scope: 'stage', run: () => store.shell.stepView('orbit-right') },
    { id: 'view.orbitUp', scope: 'stage', run: () => store.shell.stepView('orbit-up') },
    { id: 'view.orbitDown', scope: 'stage', run: () => store.shell.stepView('orbit-down') },
    { id: 'view.opposite', scope: 'stage', run: () => store.shell.stepView('opposite') },
    { id: 'view.rollLeft', scope: 'stage', run: () => store.shell.stepView('roll-left') },
    { id: 'view.rollRight', scope: 'stage', run: () => store.shell.stepView('roll-right') },
    { id: 'view.all', scope: 'stage', run: () => store.shell.frameAll() },
    { id: 'view.zoomIn', scope: 'stage', run: () => store.shell.zoomView(1) },
    { id: 'view.zoomOut', scope: 'stage', run: () => store.shell.zoomView(-1) },
