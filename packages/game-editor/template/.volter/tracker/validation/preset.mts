// A game-roadmap preset. Each issue is one game-level feature arc with a title,
// one lifecycle status, and one required Summary description. Acceptance
// criteria remain parseable for future use but are not scaffolded: the initial
// roadmap task is the complete playable prototype, proven by the game's
// autoplay record and analytics/readouts rather than a checklist here.
//
...
export const SpecIssueSchema = z.object({
  ...
  summary: z.string().min(1),                         // core; every roadmap task explains its arc
...
const SPEC_RULES = [
  rule<SpecRoot, Record<string, never>>({
    code: 'roadmap_empty',
    severity: 'warning',
    select: (m) => (m.root.issues.length === 0 ? [{}] : []),
    message: () =>
      'Roadmap is empty. Before coding, add one carefully scoped task for the complete playable prototype, including game-owned autoplay and gameplay analytics.',
  }),
  // Evidence is opt-in for a game: a checked box is done, and the real proof lives in the
  // playtest record. Restore ztrack's `passed_ac_missing_evidence` rule here to require it.
...
export const SpecPreset: Preset<SpecRoot> = {
  ...
  rules: SPEC_RULES,
  // A new task starts with its feature-arc description and one lifecycle status supplied by
  // the document record. Acceptance criteria are intentionally not invented at this stage.
  scaffold: (_title) =>
    'Summary: Describe the complete playable prototype or later major game feature.\n',
};
