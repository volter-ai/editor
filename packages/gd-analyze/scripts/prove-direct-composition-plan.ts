...
import {
  GODOT_4_7_LIFECYCLE_COMPARISON_SHA256,
  GODOT_4_7_LIFECYCLE_IMPLEMENTATION_SHA256,
  GODOT_4_7_LIFECYCLE_INPUT_SHA256,
  GODOT_4_7_LIFECYCLE_OBSERVED_OUTPUT_SHA256,
  GODOT_4_7_PROJECT_STARTUP_COMPARISON_SHA256,
  GODOT_4_7_PROJECT_STARTUP_IMPLEMENTATION_SHA256,
  GODOT_4_7_PROJECT_STARTUP_INPUT_SHA256,
  GODOT_4_7_PROJECT_STARTUP_OBSERVED_OUTPUT_SHA256,
} from '../src/translate/data/authority/godot-4.7-lifecycle';
...
function lifecycleImplementationDigest(): string {
...
function startupImplementationDigest(): string {
  const monorepo = path.resolve(import.meta.dirname, '..', '..', '..');
  return sha256(
    [
...
try {
  ...
  const lifecycleEvidence = {
    ...
    implementation: lifecycleImplementationDigest(),
    observed: sha256(JSON.stringify(nativeLifecycle)),
    comparison: sha256(lifecycleComparison),
  };
  if (
    lifecycleEvidence.input !== GODOT_4_7_LIFECYCLE_INPUT_SHA256 ||
    lifecycleEvidence.implementation !== GODOT_4_7_LIFECYCLE_IMPLEMENTATION_SHA256 ||
    lifecycleEvidence.observed !== GODOT_4_7_LIFECYCLE_OBSERVED_OUTPUT_SHA256 ||
    lifecycleEvidence.comparison !== GODOT_4_7_LIFECYCLE_COMPARISON_SHA256
  ) {
    throw new Error(`lifecycle evidence changed: ${JSON.stringify(lifecycleEvidence)}`);
  }
  ...
  const startupEvidence = {
    ...
    implementation: startupImplementationDigest(),
    observed: sha256(JSON.stringify(nativeStartup)),
    comparison: sha256(startupComparison),
  };
  if (
    startupEvidence.input !== GODOT_4_7_PROJECT_STARTUP_INPUT_SHA256 ||
    startupEvidence.implementation !== GODOT_4_7_PROJECT_STARTUP_IMPLEMENTATION_SHA256 ||
    startupEvidence.observed !== GODOT_4_7_PROJECT_STARTUP_OBSERVED_OUTPUT_SHA256 ||
    startupEvidence.comparison !== GODOT_4_7_PROJECT_STARTUP_COMPARISON_SHA256
  ) {
    throw new Error(`project-startup evidence changed: ${JSON.stringify(startupEvidence)}`);
  }
  process.stdout.write(
    `${JSON.stringify({ verdict: 'exact-match', native: nativeValue, lifecycle: { native: nativeLifecycle, target: targetLifecycle, claimIds: translation.plan.sceneModules.evidenceClaimIds, registryDigest: translation.plan.sceneModules.semanticClaimRegistryDigest }, startup: { native: nativeStartup, target: targetStartup, claimIds: translation.plan.projectData.evidence.projectStartupClaimIds, registryDigest: translation.plan.projectData.evidence.lifecycleRegistryDigest }, composition: observed, translationPlan: { version: translation.plan.version, artifacts: translation.plan.artifacts.length, deviations: translation.plan.deviations.length }, codeArtifacts: codeArtifacts.map(({ path: artifactPath, digest, origin }) => ({ path: artifactPath, digest, origin })), sceneArtifact: { path: sceneArtifact.path, digest: sceneArtifact.digest, origin: sceneArtifact.origin }, projectArtifacts: { count: projectArtifacts.length, primary: ['src/world.tsx', 'vgai.project.json', 'package.json', 'package-lock.json', 'src/main.ts', 'vgai.adapter.ts', 'tsconfig.json', 'vite.config.ts'].map((artifactPath) => ({ path: artifactPath, digest: byPath.get(artifactPath)?.digest })) }, packageRequirements: translation.plan.projectData.requirements.packages.length, capabilityRequirements: translation.plan.projectData.requirements.capabilities.length, evidence: composition.evidence })}\n`,
  );
} finally {
