 ...
 * page } from '@volter/game-live'` and call immediately — each call transparently
    ...
    apply(_target, _thisArg, args) {
      return resolveRoot().then((root) => {
        ...
        if (!isFunction(fn)) {
          ...
          throw new TypeError(`@volter/game-live: ${label} is not a function on the connected session.`);
        }
