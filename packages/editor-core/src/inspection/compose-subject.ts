...
function selectionPreviewRenderer(input: {
  readonly media: InspectionNodeMedia | null;
  ...
  // A node its medium draws is previewed by that medium — or not at all, when
  // the medium shows it elsewhere (a camera's view is the viewport's).
  if (input.media) {
    return input.media.preview
      ? input.media.preview({
...
export interface ComposeInspectionInput {
  ...
  /**
   * What this node looks like, answered by the medium that draws it
   * (`@volter/editor-sdk/kit/inspection-node-media`), or `null` when none does.
   * A medium's answer decides the Preview section and the instanced note; it
   * is resolved from the live object, because "previewable" and "renders
   * honestly" have to be the same fact.
   */
  readonly media: InspectionNodeMedia | null;
...
export function composeInspectionSubject(input: ComposeInspectionInput): InspectionSubject {
  ...
  const componentPicture =
    input.surface === 'canvas' && media === null && input.stories
      ? contentEntryForComponent({
          name: componentName,
  ...
  if (!claimed.has(TRANSFORM_SECTION_ID) && transformDimensions !== null) {
    ...
    const instanced = media?.instanced ?? null;
