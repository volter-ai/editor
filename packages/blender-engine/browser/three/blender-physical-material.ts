...
export function applyPhysicalMaterial(material: THREE.MeshPhysicalMaterial, input?: Physical,
  ...
  if (!values) {
    ...
    material.onBeforeCompile = shader => {
      // Three declares channels 1..3 only for its own maps, and none past 3.
      // Blender has eight named UV maps beside the default channel (9 is the
      // missing layer's zeros). Declare only the channels this draw reads: a
      // driver may count every declared attribute against its sixteen, and
      // six idle ones leave a graph with a few attributes no room.
      const read = new Set([material.map, material.roughnessMap, material.normalMap]
        .flatMap(texture => (texture ? [texture.channel] : [])).concat(graphUvChannels(material)));
      shader.vertexShader = [...read].filter(i => i > 0).sort((a, b) => a - b)
